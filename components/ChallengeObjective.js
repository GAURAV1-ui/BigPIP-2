...
const Challenge = () => {
  ...
  useEffect(() => {
    axios
      .get(
        "https://bigpip-cms.up.railway.app/api/home?populate=evaluation.descriptionPoints"
      )
      .then((res) => {
    ...
    <div className={styles.objective}>
      <div className={styles.objective_content}>
        ...
        <div className={styles.objective_cards}>
          <div
            className={`${styles.objective_card} ${styles.objective_card_standard}`}
          >
            <div className={styles.objective_card_heading}>
              <h3>Standard Evaluation</h3>
              <p className={styles.objective_card_heading_p}>
          ...
          <div
            className={`${styles.objective_card} ${styles.objective_card_onestep}`}
          >
            <div className={styles.objective_card_heading}>
              <h3>1-step Evaluation</h3>
              <p className={styles.objective_card_heading_p}>
                Get Funded by skipping one step
              </p>
            </div>
            <div className={styles.objective_card_content}>
              {challenge1.map((data) => (
...
export async function getServerSideProps() {
  const res = await fetch(
    `https://bigpip-cms.up.railway.app/api/home?populate=*`
  );
